...
bool InitializeWindow(VkExtent2D size, bool fullScreen = false, bool isResizable = true, bool limitFrameRate = true)
{
    ...
    {
        std::cout << "添加扩展 ： " << extensionNames[i] << "\n";
        graphicsBase::Base().AddInstanceExtension(extensionNames[i]);
    }
    std::cout << "添加扩展 ： " << VK_KHR_SWAPCHAIN_EXTENSION_NAME << "\n";
    ...
    if (VkResult result = glfwCreateWindowSurface(graphicsBase::Base().Instance(), glfw_window, nullptr, &surface))
    {
